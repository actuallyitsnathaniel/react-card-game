...
class Card extends React.Component {
  ...
  // Clearing timeout is MANDATORY. or it will bug out
  ...
  render() {
    ...
    return (
      <TouchableOpacity onPress={onPress}>
        <Animated.View style={styles.card}>{displayImage}</Animated.View>
      </TouchableOpacity>
    );
  }
}
...
const getRowOffset = index => {
  // OPEN FOR IMPROVEMENT, by word of Spencer
  switch (index) {
    case 0:
      return 1.5;
    case 1:
...
class Row extends React.Component {
  offset = new Animated.Value(CARD_HEIGHT * getRowOffset(this.props.index)); // Animated view animates cards
    ...
    this.timeout = setTimeout(() => {
      Animated.parallel([
        Animated.timing(this.offset, {
          toValue: 0,
          duration: 1050,
          useNativeDriver: true // THIS IS IMPORTANT TO HAVE FOR ALL ANIMATIONS
        }),
